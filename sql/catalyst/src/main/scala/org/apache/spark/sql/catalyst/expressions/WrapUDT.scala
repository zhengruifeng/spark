    ...
    if (child.dataType == to.sqlType) {
      ...
      DataTypeMismatch(
        ...
        messageParameters = Map(
          ...
          "requiredType" -> toSQLType(to.sqlType),
          "inputSql" -> toSQLExpr(child),
          "inputType" -> toSQLType(child.dataType)))
    }
  }

  override def dataType: DataType = to
