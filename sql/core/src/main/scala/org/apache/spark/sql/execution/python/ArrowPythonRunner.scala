...
object ArrowPythonRunner {
  ...
  def getPythonRunnerConfMap(conf: SQLConf): Map[String, String] = {
    ...
    ).foreach(c => confMap.update(c.key, conf.getConf(c).toString))

    conf.pythonUDFArrowConcurrencyLevel.foreach { v =>
      confMap.update(SQLConf.PYTHON_UDF_ARROW_CONCURRENCY_LEVEL.key, v.toString)
    }
    conf.pythonUDFProfiler.foreach { p =>
      confMap.update(SQLConf.PYTHON_UDF_PROFILER.key, p)
    }
    conf.pythonDataSourceProfiler.foreach { p =>
      confMap.update(SQLConf.PYTHON_DATA_SOURCE_PROFILER.key, p)
    }
    confMap.toMap
  }
}
