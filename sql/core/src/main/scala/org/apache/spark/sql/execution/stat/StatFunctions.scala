...
object StatFunctions extends Logging {
  ...
  def multipleApproxQuantiles(
    ...
    val accuracy: Long = if (relativeError == 0) {
      Int.MaxValue
    } else {
      val relativeErrorNum = BigDecimal(relativeError)
      relativeErrorNum.signum * Math.min(Math.max(
        (BigDecimal(1.0) / relativeErrorNum.abs).rounded.longValue, 1L), Int.MaxValue)
    }

    val columns = cols.map { c =>
      val dataType = df.resolve(c).dataType
      require(dataType.isInstanceOf[NumericType],
        s"Quantile calculation for column $c with data type $dataType" +
          " is not supported.")
      nvl(
        approx_percentile(
          nanvl(col(c).cast(DoubleType), lit(null)),
          lit(probabilities.toArray),
          lit(accuracy)),
        lit(Array.emptyDoubleArray))
    }

    val row = df.select(columns: _*).head()
    Seq.tabulate(row.size)(i => row.getSeq[Double](i))
  }
