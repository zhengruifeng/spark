...
private[sql] object ArrowUtils {
   ...
   */
  private def failDuplicateFieldNames(dt: DataType): Unit = {
    dt match {
      case st: StructType =>
        ...
        st.fields.foreach {field => failDuplicateFieldNames(field.dataType)}
      case arr: ArrayType =>
        failDuplicateFieldNames(arr.elementType)
      case map: MapType =>
        failDuplicateFieldNames(map.keyType)
        failDuplicateFieldNames(map.valueType)
      case udt: UserDefinedType[_] =>
        failDuplicateFieldNames(udt.sqlType)
      case _ =>
    }
  }

  def checkDuplicateFieldNames(schema: StructType): StructType = {
    failDuplicateFieldNames(schema)
    schema
  }
