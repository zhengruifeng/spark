...
import org.apache.spark.sql.connect.planner.{InvalidCommandInput, LiteralExpressionProtoConverter, SaveModeConverter, SparkConnectPlanner}
...
private[connect] class SparkConnectCommandHandler(session: SparkSession) {
  ...
  private def handleSqlCommand(
    ...
    val df = session.sql(
      ...
      getSqlCommand.getArgsMap.asScala
        .mapValues(LiteralExpressionProtoConverter.toCatalystExpression)
        .toMap)
