    ...
    def pivot(self, pivot_col: str, values: Optional[List["LiteralType"]] = None) -> "GroupedData":
        ...
        if not isinstance(pivot_col, str):
            raise PySparkTypeError(
                ...
                messageParameters={"expected_type": "a str", "arg_name": "pivot_col", "arg_type": type(pivot_col).__name__},
            )
        ...
        if values is not None:
            if not isinstance(values, list):
                raise PySparkTypeError(
                    errorClass="NOT_EXPECTED_TYPE",
                    messageParameters={"expected_type": "a list", "arg_name": "values", "arg_type": type(values).__name__},
                )
            for v in values:
                if not isinstance(v, (bool, float, int, str)):
                    raise PySparkTypeError(
                        errorClass="NOT_EXPECTED_TYPE",
                        messageParameters={"expected_type": "a bool, float, int or str", "arg_name": "value", "arg_type": type(v).__name__},
                    )
