...
def broadcast(df: "DataFrame") -> "DataFrame":
    ...
    if not isinstance(df, DataFrame):
        raise PySparkTypeError(
            errorClass="NOT_EXPECTED_TYPE",
            messageParameters={"expected_type": "a DataFrame", "arg_name": "df", "arg_type": type(df).__name__},
        )
...
def when(condition: Column, value: Any) -> Column:
    ...
    if not isinstance(condition, Column):
        raise PySparkTypeError(
            errorClass="NOT_EXPECTED_TYPE",
            messageParameters={"expected_type": "a Column", "arg_name": "condition", "arg_type": type(condition).__name__},
        )
...
def from_csv(
    ...
    if isinstance(schema, Column):
        ...
        raise PySparkTypeError(
            errorClass="NOT_EXPECTED_TYPE",
            messageParameters={"expected_type": "a Column or str", "arg_name": "schema", "arg_type": type(schema).__name__},
        )
...
def from_json(
    ...
    if isinstance(schema, (str, Column)):
        ...
        raise PySparkTypeError(
            errorClass="NOT_EXPECTED_TYPE",
            messageParameters={"expected_type": "a Column, str or DataType", "arg_name": "schema", "arg_type": type(schema).__name__},
        )
...
def from_xml(
    ...
    if isinstance(schema, (str, Column)):
        ...
        raise PySparkTypeError(
            errorClass="NOT_EXPECTED_TYPE",
            messageParameters={"expected_type": "a StructType, Column or str", "arg_name": "schema", "arg_type": type(schema).__name__},
        )
...
def schema_of_csv(csv: Union[str, Column], options: Optional[Mapping[str, str]] = None) -> Column:
    ...
    if not isinstance(csv, (str, Column)):
        raise PySparkTypeError(
            errorClass="NOT_EXPECTED_TYPE",
            messageParameters={"expected_type": "a Column or str", "arg_name": "csv", "arg_type": type(csv).__name__},
        )
...
def schema_of_json(json: Union[str, Column], options: Optional[Mapping[str, str]] = None) -> Column:
    ...
    if not isinstance(json, (str, Column)):
        raise PySparkTypeError(
            errorClass="NOT_EXPECTED_TYPE",
            messageParameters={"expected_type": "a Column or str", "arg_name": "json", "arg_type": type(json).__name__},
        )
...
def schema_of_xml(xml: Union[str, Column], options: Optional[Mapping[str, str]] = None) -> Column:
    ...
    if not isinstance(xml, (str, Column)):
        raise PySparkTypeError(
            errorClass="NOT_EXPECTED_TYPE",
            messageParameters={"expected_type": "a Column or str", "arg_name": "xml", "arg_type": type(xml).__name__},
        )
...
def slice(
    ...
    if isinstance(start, (Column, str)):
        ...
        raise PySparkTypeError(
            errorClass="NOT_EXPECTED_TYPE",
            messageParameters={"expected_type": "a Column, int or str", "arg_name": "start", "arg_type": type(start).__name__},
        )
    ...
    if isinstance(length, (Column, str)):
        ...
        raise PySparkTypeError(
            errorClass="NOT_EXPECTED_TYPE",
            messageParameters={"expected_type": "a Column, int or str", "arg_name": "length", "arg_type": type(length).__name__},
        )
...
def overlay(
    ...
    if not isinstance(pos, (int, str, Column)):
        raise PySparkTypeError(
            errorClass="NOT_EXPECTED_TYPE",
            messageParameters={"expected_type": "a Column, int or str", "arg_name": "pos", "arg_type": type(pos).__name__},
        )
    len = _enum_to_value(len)
    if len is not None and not isinstance(len, (int, str, Column)):
        raise PySparkTypeError(
            errorClass="NOT_EXPECTED_TYPE",
            messageParameters={"expected_type": "a Column, int or str", "arg_name": "len", "arg_type": type(len).__name__},
        )
...
def window(
    ...
    if startTime is not None and not isinstance(startTime, str):
        raise PySparkTypeError(
            errorClass="NOT_EXPECTED_TYPE",
            messageParameters={"expected_type": "a str", "arg_name": "startTime", "arg_type": type(startTime).__name__},
        )
...
def session_window(timeColumn: "ColumnOrName", gapDuration: Union[Column, str]) -> Column:
    ...
    if gapDuration is None or not isinstance(gapDuration, (Column, str)):
        raise PySparkTypeError(
            errorClass="NOT_EXPECTED_TYPE",
            messageParameters={"expected_type": "a Column or str", "arg_name": "gapDuration", "arg_type": type(gapDuration).__name__},
        )
...
def _ensure_column_or_name(arg: Optional[Any]) -> "ColumnOrName":
    if not isinstance(arg, (Column, str)):
        raise PySparkTypeError(
            errorClass="NOT_EXPECTED_TYPE",
            messageParameters={"expected_type": "a Column or str", "arg_name": "arg", "arg_type": type(arg).__name__},
        )
    return arg
...
def assert_true(col: "ColumnOrName", errMsg: Optional[Union[Column, str]] = None) -> Column:
    ...
    if not isinstance(errMsg, (str, Column)):
        raise PySparkTypeError(
            errorClass="NOT_EXPECTED_TYPE",
            messageParameters={"expected_type": "a Column or str", "arg_name": "errMsg", "arg_type": type(errMsg).__name__},
        )
