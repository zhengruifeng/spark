...
class DataFrame(ParentDataFrame, PandasMapOpsMixin, PandasConversionMixin):
    ...
    def replace(  # type: ignore[misc]
        ...
        _subset, _rep_dict = self._prepare_args_for_replace(to_replace, value, subset)
        if _subset is None:
            return DataFrame(self._jdf.na().replace("*", _rep_dict), self.sparkSession)
        else:
            return DataFrame(
                self._jdf.na().replace(self._jseq(_subset), self._jmap(_rep_dict)),
                self.sparkSession,
            )
